...
### 8.3 `git-cleanup` 的价值不在“删分支”，而在“安全收尾”

**问题**

一开始我以为 Git 清理只是“把做完的分支删掉”，但在真正收尾 `codex/imagelightbox` 这类任务时，问题比预期复杂得多。

当时我遇到的实际情况是：

- 目标分支已经做完并推送，但它还被一个独立 worktree 占用，不能直接删除。
- 检查主仓库时，发现本地 `main` 不是干净基线，而是已经和 `origin/main` 分叉，处于 `ahead 1, behind 3` 的状态。
- 这意味着“本地 merged”并不等于“已经安全进入远端主线”，如果继续凭感觉清理，就有可能删掉仍然需要保留的本地分支或在错误基线上操作。

这次经历让我意识到，随着 Portfolio2026 开始长期使用 `main + 多个 codex worktree` 的协作方式，Git 清理已经不是简单的“删掉做完的分支”：

- 有些分支虽然 merged 到本地 `main`，但还不一定已经进入 `origin/main`。
- 有些分支还被 worktree 占用，不能直接删除。
- `main` 本身也可能已经和 `origin/main` 分叉，如果不先看清状态，就容易在错误基线上做清理。

如果没有一套固定流程，清理动作看起来很小，但实际风险并不低。

**解决方式**

把这类收尾动作沉淀成 `git-cleanup` skill，并且继续迭代成更适合当前工作流的版本。这个 skill 的价值不是“帮我执行 Git 命令”，而是把清理前真正该看的风险信号固定下来。

这次迭代补了三个核心能力：

- 清理前先检查 `main` / `origin/main` 的健康状态，明确是 ahead、behind 还是 diverged。
- 删除候选分支前，确认它是否已经进入 `origin/main`，避免只 merged 到本地 `main` 就被误删。
- 显式标出 branch 和 worktree 的占用关系，先移除 worktree，再删除分支。

这样 `git-cleanup` 就从“基础清理脚本”变成了“多 worktree 协作模式下的安全收尾流程”。

**沉淀原则**

协作型 skill 的价值，往往不在自动化本身，而在把危险判断前移，把真正该看的状态固定成检查清单。
