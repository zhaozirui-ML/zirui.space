...
本文档只记录这个项目的 design system 本体，不负责作品集页面叙事策略和高层品牌判断。
...
- 当任务主要在判断 token、theme、组件职责、共享视觉语义和系统层 ownership 时，优先阅读当前文档
- 当任务主要在判断案例叙事、页面节奏、图文关系和 Figma 页面级落地规则时，转到 `design.md`
- 当任务主要在判断受众、品牌气质、长期方向和高层设计原则时，转到 `docs/design-context.md`
- 当前文档只负责 system layer：token、组件、主题分层和职责边界

## 1. System-Level Non-Negotiables

### 1.1 字体接入

- 字体必须通过项目级全局方式正式接入，例如 layout、全局变量或 design token。
- 严禁在页面或组件里分散声明字体，或依赖本机字体、临时字体配置来完成正式方案。
- 可以先做临时占位预览，但一旦确认采用，必须立即收敛成正式接入方案。
- 引入新字体时，必须先说明三件事：
  - 来源
  - 主要用途
  - 影响范围
...
- 新增、修改或扩展颜色时，严禁直接使用 `#fff`、`#000`、`rgb()`、`rgba()` 等硬编码颜色值作为新增方案。
- 颜色必须优先使用 design system 或 portfolio theme 中已经存在的 color token / semantic token。
- 如果现有 token 不适合当前语义，不要先写临时颜色。
- 正确流程是先说明缺少什么颜色语义，再决定是否补充到 design token。
...
### 5.2 Portfolio Theme Layer 主要解决什么
